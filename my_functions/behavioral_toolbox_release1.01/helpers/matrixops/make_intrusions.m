function [intrusions_matrix] = make_intrusions(recalls_itemnos,pres_itemnos,subjects)
% INTRUSIONS makes an intrusions_matrix, formatted to the standard
% intrusions field of a data structure
%
%
% [intrusions_matrix] = make_intrusions(recalls_itemnos,pres_itemnos,subjects)
%
% INPUTS:
% recalls_itemnos:   a matrix whose elements are indices of recalled
...
%                    recalls_itemnos with a subject number
...
%   element-for-element with the recalls_itemnos, the exclusion of
%   repeats from the recalls_itemnos would prevent this from
...
elseif size(pres_itemnos, 1) ~= length(subjects)
...
for rows = 1:size(recitemnos,1)
    ...
    if ~isempty(is_intrusion)
        % if we're on the first list, the intrusion must be an XLI.
        if isequal(rows,1)
...
% items. if so, record the list lag. otherwise, it's an XLI.
